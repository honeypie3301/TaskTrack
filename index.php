...
if (isset($_POST['add_task'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO tasks (user_id, title, description, deadline, priority, status) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssss", $user_id, $title, $description, $deadline, $priority, $status);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['update_task'])) {
    ...
    $stmt = $conn->prepare("UPDATE tasks SET title = ?, description = ?, deadline = ?, priority = ?, status = ? WHERE $id_column = ? AND user_id = ?");
    $stmt->bind_param("sssssii", $title, $description, $deadline, $priority, $status, $task_id, $user_id);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM tasks WHERE $id_column = ? AND user_id = ?");
